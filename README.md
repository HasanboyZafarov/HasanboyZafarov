<div align="center">

# 👋 Hasanboy Zafarov

**Frontend Developer · React Specialist · UI Craftsman**

<img src="https://readme-typing-svg.demolab.com?font=Fira+Code&size=18&duration=2500&pause=1000&color=A78BFA&center=true&vCenter=true&width=600&lines=Building+clean+UIs+with+React+%26+TypeScript;Turning+Figma+designs+into+pixel-perfect+code;Full-stack+journey%3A+frontend+→+backend;Always+shipping%2C+always+improving" alt="Typing SVG" />

</div>

---

## 👤 About Me

- 🎯 **Frontend Developer** specializing in React.js, TypeScript, and modern UI patterns
- 🎓 Cybersecurity student @ **TUIT Nukus Branch** · Andijan, Uzbekistan 🇺🇿
- 🧠 Writing clean, scalable, maintainable code — not just what works
- ♟️ Chess enthusiast. Problem solver. Continuous learner.
- 🚀 Career goal: Build world-class products as a Senior Full-Stack Developer

---

## 🛠️ Tech Stack

<div align="center">

**Languages & Markup**

<img src="https://skillicons.dev/icons?i=html,css,js,ts&theme=dark" />

**Frontend Frameworks & Tools**

<img src="https://skillicons.dev/icons?i=react,tailwind,vite,figma,git&theme=dark" />

**Developer Tools**

<img src="https://skillicons.dev/icons?i=vscode,github,npm&theme=dark" />

</div>

---

## 📊 GitHub Stats

<div align="center">

![GitHub Stats](./stats/github-stats.svg)

</div>

### 📊 Stats

<!-- STATS:START -->
📦 All Repos: 45  |  🔒 Private: 23  |  🌐 Public: 22
<!-- STATS:END -->

*Stats auto-updated daily via GitHub Actions*

---

## 🎯 Core Competencies

- ⚛️ **React** — Hooks, Context API, performance optimization, component patterns
- 🎨 **UI/UX Implementation** — Figma → Code, responsive design, accessibility
- 📦 **State Management** — Zustand, custom hooks
- 🎭 **Styling** — Tailwind CSS, SCSS, CSS-in-JS

---

## 📬 Connect With Me

<div align="center">

[![Email](https://img.shields.io/badge/Email-dewozto1337%40gmail.com-EA4335?style=for-the-badge&logo=gmail&logoColor=white)](mailto:dewozto1337@gmail.com)
[![GitHub](https://img.shields.io/badge/GitHub-%40HasanboyZafarov-181717?style=for-the-badge&logo=github&logoColor=white)](https://github.com/HasanboyZafarov)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-Hasanboy%20Zafarov-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)](https://linkedin.com/in/hasanboyzafarov)

</div>

---

<div align="center">

![Profile Views](https://komarev.com/ghpvc/?username=HasanboyZafarov&style=flat-square&color=a78bfa&label=Profile+Views)

**"Consistency beats intensity. 1% better every day."** 💪

</div>
